...
def load(path_in, config, sep=','):
    ...
    df_out.Account = 'Discovery'
    df_out.Currency = config['currency']
    df_out.Amount = [c - d for c,d in zip(pd.to_numeric(df['Credit'].str.replace('$', ''), errors='coerce'), pd.to_numeric(df['Credit'].str.replace('$', ''), errors='coerce'))]
