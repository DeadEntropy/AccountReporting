...
def load(path_in, config, *args):
    ...
    activity = pd.read_csv(path_in_activity, parse_dates=["Date"], date_format="%d-%b-%y").rename(
    ...
    for account in df.Pot.unique():
        ...
        df_out["Amount"] = [
            s if desc == "Purchase" else -s if desc == "Sale" else -tv if desc == "Fee" else tv
            for s, tv, desc in zip(acc_activ["No. Shares"], acc_activ["Total Value"], acc_activ["Description"])
        ]
        df_out["Currency"] = [
            asset if desc in ["Purchase", "Sale"] else "GBP" for asset, desc in zip(acc_activ["Asset Code"], acc_activ["Description"])
        ]
        df_out["Subcategory"] = acc_activ["Description"]
        df_out["Memo"] = [
            f"Nutmeg: {account} Fund {desc}" if desc in ["Purchase", "Sale"] else desc
            for asset, desc in zip(acc_activ["Asset Code"], acc_activ["Description"])
        ]
        ...
        df_out_cash["Amount"] = [
            -tv if desc == "Purchase" else tv for tv, desc in zip(acc_activ_cash["Total Value"], acc_activ_cash["Description"])
        ]
        df_out_cash["Currency"] = "GBP"
        df_out_cash["Subcategory"] = acc_activ_cash["Description"]
        df_out_cash["Memo"] = [
            f"Nutmeg: {account} Fund {desc}" for asset, desc in zip(acc_activ_cash["Asset Code"], acc_activ_cash["Description"])
        ]
