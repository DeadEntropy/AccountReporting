...
def get_transaction(df: str, market: Market, proportion: {}, switch: {}, ref_currency: str, memo: str, date_format: str = '%d/%m/%Y'):
    ...
    for fund_name in list_of_funds:
        ...
        df_fund['Amount'] = df_transaction[f'Amount'] / df_fund['Price'] *\
        ...
        df_cash_impact['Amount'] = -df_transaction[f'Amount'] * [proportion[get_key(proportion, date)][fund_name]\
                                                                 for date in df_transaction.index]
    ...
    for k, v in switch.items():
        for fund_name, fund_units in v.items():
            ...
            result = result.append(pd.DataFrame([row], columns=result.columns))
