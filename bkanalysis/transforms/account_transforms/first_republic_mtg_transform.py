...
def get_payment_breakdown(sorted_dates: list, config) -> pd.DataFrame:
    ...
    interest_payment = {d: float(round(sch.interest, 2)) for d, sch in zip(sorted_dates, loan.schedule()[1 : len(sorted_dates) + 1])}
...
def load(path_in, config, *args):
    ...
    df_out = pd.concat([df for df in [df_out, df_payment_breakdown, df_payment_adjustment] if len(df) > 0]).sort_values(
