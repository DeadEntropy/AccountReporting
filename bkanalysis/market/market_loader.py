...
class MarketLoader:
    ...
    def __init__(self, config=None):
        ...
        if "source_from_nutmeg" in self.config["Market"]:
            ...
            if (
                "Nutmeg" not in self.config
                or "path_activity" not in self.config["Nutmeg"]
            ):
                raise Exception(
                    "Nutmeg.path_activity must be passed in is source_from_nutmeg is enabled."
                )
    ...
    def load(self, instruments, ref_currency: str, period: str):
        ...
        symbols = [
            (
                self.get_symbol(instr, ref_currency)
                if instr not in self.source_map.keys()
                else instr
            )
        ...
        currencies_from_yahoo = list(
            set(
                [
                    self.get_currency(symbol)
                    for symbol in symbols
                    if symbol not in self.source_map.keys()
                ]
            )
        )
        symbols = list(
            set(
                [
                    f"{ccy}{ref_currency}=X"
                    for ccy in currencies_from_yahoo
                    if ccy != ref_currency and ccy is not None
                ]
                + symbols
            )
        )
        ...
        values = {
            symbol: self.get_history(symbol, period)
            for symbol in symbols
    ...
    def get_history(self, symbol: str, period: str):
        if symbol in self.source_map:
            if self.source_map[symbol][0] == SOURCE.YAHOO:
                logging.info(
                    f"Getting {period} of data for {symbol} from {SOURCE.YAHOO}"
                )
                return self.get_history_from_yahoo(symbol, period)
            elif self.source_map[symbol][0] == SOURCE.FILE:
                logging.info(
                    f"Getting {period} of data for {symbol} from {SOURCE.FILE}"
                )
                ...
                logging.info(
                    f"Getting {period} of data for {symbol} from {SOURCE.HARDCODED}"
                )
                if len(self.source_map[symbol]) != 3:
                    raise Exception("source is HARDCODED but not VALUE was passed in.")
                return self.get_history_from_hardcoded(
                    self.source_map[symbol][1], self.source_map[symbol][2]
                )
            elif self.source_map[symbol][0] == SOURCE.NUTMEG:
                logging.info(
                    f"Getting {period} of data for {symbol} from {SOURCE.NUTMEG}"
                )
                if len(self.source_map[symbol]) != 1:
                    raise Exception(
                        "source is NUTMEG but too many arguments were passed in."
                    )
                return self.get_history_from_nutmeg(self.nutmeg_path, symbol)
            else:
                raise Exception(
                    f"{self.source_map[symbol][0]} source is not supported."
                )
    ...
    def get_history_from_hardcoded(values: dict, currency: str):
        ...
        return {
            datetime.strptime(date, MarketLoader._FILE_DATE_FORMAT): Price(
                close, currency
            )
    ...
    def get_history_from_nutmeg(path: str, instr: str):
        ...
        return {
            ...
            for (date, close) in pd.pivot_table(
                df_instr, index="Date", values="Share Price", aggfunc="first"
            )["Share Price"].items()
        }
    ...
    def get_history_from_file(path: str):
        ...
        df["Unit Price Date"] = pd.to_datetime(
            df["Unit Price Date"], format=MarketLoader._FILE_DATE_FORMAT
        )
        return {
            date: Price(close, currency)
            for (date, close) in df.set_index("Unit Price Date")["Unit Price"].items()
        }
    ...
    def get_history_from_yahoo(symbol: str, period: str):
        ...
        return {
            date: Price(close, currency)
            for (date, close) in mp.get_history(symbol, period).Close.items()
        }
