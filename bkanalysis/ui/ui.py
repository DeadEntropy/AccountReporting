...
def load_transactions(
    ...
    df = pr.process(
        df_raw, ignore_overrides=ignore_overrides, remove_offsetting=remove_offsetting
    )
    if save_to_csv:
        pr.save(df)
    pr.__del__()
...
def add_no_capital_column(df):
    df[CAPITAL_GAIN] = [
        next((float(x[1]) for x in l if x[0] == "CAPITAL"), 0.0)
        for l in df[MEMO_MAPPED]
    ]
    df[CUMULATED_CAPITAL_GAIN] = df.groupby(["Account", "Currency"])[
        CAPITAL_GAIN
    ].transform(pd.Series.cumsum)
    df[CUMULATED_AMOUNT_CCY_EXCL_CAPITAL] = (
        df[CUMULATED_AMOUNT_CCY] - df[CUMULATED_CAPITAL_GAIN]
    )
...
def transactions_to_values(df):
    ...
    df.Date = [
        dt.datetime(old_date.year, old_date.month, old_date.day) for old_date in df.Date
    ]

    # Ensure there all (Account, Currency, Date) tuples are unique
    df.drop(
        df.columns.difference(
            ["Account", "Currency", DATE, MEMO_MAPPED, AMOUNT, "Type"]
        ),
        axis=1,
        inplace=True,
    )
    ...
    df[MEMO_MAPPED] = [
        [(i1, i2, t) for (i1, i2, t) in zip(l1, l2, types)]
    ...
    df[CUMULATED_AMOUNT] = df.groupby(["Account", "Currency"])[AMOUNT].transform(
        pd.Series.cumsum
    )
    ...
    index = list(
        itertools.product(
            *[
                list(
                    set(list(df.reset_index().set_index(["Account", "Currency"]).index))
                ),
                date_range,
            ]
        )
...
def compute_price(
    ...
    values = market_loader.load(
        df.reset_index().Currency.unique(), ref_currency, period
    )
    ...
    price_in_currency = [
        (
            market.get_price_in_currency(
                (
                    ml.MarketLoader.get_symbol(instr, ref_currency)
                    if instr not in market_loader.source_map.keys()
                    else instr
                ),
                date,
                ref_currency,
    ...
    df[MEMO_MAPPED] = [
        [(k, v * p, t) for (k, v, t) in memo] if memo != "" else []
        for (memo, p) in zip(df[MEMO_MAPPED], price_in_currency)
    ]
    ...
    df[MEMO_MAPPED] = [
        memo + [("CAPITAL", c, "C")] if c != 0 else memo
        for (memo, c) in zip(df[MEMO_MAPPED], pd.concat(cap_series))
    ]
...
def add_mappings(df, ref_currency, config=None):
    ...
    df_exp[AMOUNT_CCY] = [
        memoMapped[1] if not isinstance(memoMapped, float) else 0
        for memoMapped in df_exp[MEMO_MAPPED]
    ]
    df_exp[MEMO_MAPPED] = [
        memoMapped[0] if not isinstance(memoMapped, float) else ""
        for memoMapped in df_exp[MEMO_MAPPED]
    ]
...
def _get_plot_data(df, date_range=None, by=CUMULATED_AMOUNT_CCY):
    ...
    if date_range is not None:
        if len(date_range) == 2:
            ...
            raise Exception(
                f"date_range is not set to a correct value ({date_range}). Expected None or String."
            )
...
def aggregate_memos(memo):
    ...
    largest_memos = [
        (k, v)
        for (k, v) in sorted_memo.items()
        if abs(v) > sorted([abs(v) for (k, v) in sorted_memo.items()], reverse=True)[19]
    ]
    remainging_amount = sum([v for k, v in sorted_memo.items()]) - sum(
        [v for k, v in largest_memos]
    )
    largest_memos = dict(
        sorted(
...
def get_annotations(df, top_items_count=10):
    l = [
        [
            [amt, d] + list(memo)
            for memo in memos
            if not memo[2] in IatIdentification.iat_types + ["C", "S"]
        ]
        for (amt, memos, d) in zip(
            df.CumulatedAmountCcyExclCapGains, df.MemoMapped, df.Date
        )
    ]
...
def plot_wealth(
    df, date_range=None, by=CUMULATED_AMOUNT_CCY, top_items_count=0, rename=None
):
    if isinstance(by, str):
        values, labels = _get_plot_data(df, date_range, by)
        fig = go.Figure(
            data=go.Scatter(x=values.index, y=values.values, hovertext=labels)
        )
    ...
    if date_range is None:
        ...
        values, labels = _get_plot_data(
            df, date_range, CUMULATED_AMOUNT_CCY_EXCL_CAPITAL
        )
    else:
        annotations = get_annotations(
            df[(df.Date > date_range[0]) & (df.Date < date_range[1])], top_items_count
        )
        values, labels = _get_plot_data(
            df, date_range, CUMULATED_AMOUNT_CCY_EXCL_CAPITAL
        )
    ...
    for annotation_date in collections.OrderedDict(sorted(annotations.items())):
        ...
        fig.add_annotation(
            ...
            ay=(
                -100
                if (i % 4) == 0
                else 50 if (i % 3) == 0 else -50 if (i % 2) == 0 else 100
            ),
            bordercolor="#c7c7c7",
            borderwidth=2,
            borderpad=4,
...
def plot_wealth_yearly(
    df, by=CUMULATED_AMOUNT_CCY_EXCL_CAPITAL, first_year=2016, last_year=None
):
...
def project(df, nb_years=11, projection_data={}):
    assert (
        CUMULATED_AMOUNT_CCY in df.columns
    ), f'Expect "{CUMULATED_AMOUNT_CCY}" to be in the columns.'
    assert DATE in df.columns, f'Expect "{DATE}" to be in the columns.'
    ...
    projection["Return"] = [
        __try_get(projection_data, acc, [0, 0, 0])[0] for acc in projection.Account
    ]
    projection["Volatility"] = [
        __try_get(projection_data, acc, [0, 0, 0])[1] for acc in projection.Account
    ]
    projection["Contribution"] = [
        __try_get(projection_data, acc, [0, 0, 0])[2] for acc in projection.Account
    ]

    r = range(0, nb_years)
    (w, w_low, w_up, w_low_ex, w_up_ex) = pj.project_full(
        projection, r, CUMULATED_AMOUNT_CCY
    )

    piv = pd.DataFrame(
        pd.pivot_table(
            df, values=CUMULATED_AMOUNT_CCY, index=DATE, columns=[], aggfunc="sum"
        ).to_records()
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=piv[DATE], y=piv[CUMULATED_AMOUNT_CCY], mode="lines", name="Wealth"
        )
    )

    projection_x = [
        pd.Timestamp(piv[DATE].values[-1] + np.timedelta64(365 * i, "D")) for i in r
    ]
    ...
    fig.update_layout(
        title="Projected Wealth with confidence interval", xaxis_title=DATE
    )
...
def project_compare(df, nb_years=11, projection_data_1={}, projection_data_2={}):
    ...
    projection_1["Return"] = [
        __try_get(projection_data_1, acc, [0, 0, 0])[0] for acc in projection_1.Account
    ]
    projection_1["Volatility"] = [
        __try_get(projection_data_1, acc, [0, 0, 0])[1] for acc in projection_1.Account
    ]
    projection_1["Contribution"] = [
        __try_get(projection_data_1, acc, [0, 0, 0])[2] for acc in projection_1.Account
    ]
    ...
    projection_2["Return"] = [
        __try_get(projection_data_2, acc, [0, 0, 0])[0] for acc in projection_2.Account
    ]
    projection_2["Volatility"] = [
        __try_get(projection_data_2, acc, [0, 0, 0])[1] for acc in projection_2.Account
    ]
    projection_2["Contribution"] = [
        __try_get(projection_data_2, acc, [0, 0, 0])[2] for acc in projection_2.Account
    ]

    r = range(0, nb_years)
    (w1, w1_low, w1_up, w1_low_ex, w1_up_ex) = pj.project_full(
        projection_1, r, CUMULATED_AMOUNT_CCY
    )
    (w2, w2_low, w2_up, w2_low_ex, w2_up_ex) = pj.project_full(
        projection_2, r, CUMULATED_AMOUNT_CCY
    )
...
def get_by(
    ...
    if by not in ["FullMasterType", "FullSubType", "FullType"]:
        raise ValueError(
            f"by must be in {['FullMasterType', 'FullSubType', 'FullType']}"
        )
    ...
    if date_range is None:
        ...
        if len(date_range) == 2:
            df_exp_date = df_exp[
                (df_exp.Date >= date_range[0]) & (df_exp.Date <= date_range[1])
            ]
        else:
            raise ValueError(
                f"date_range is not set to a correct value ({date_range}). Expected None or String."
    ...
    if key is None:
        ...
        df = pd.pivot_table(
            df_exp_date[
                (df_exp_date[by] == key)
                & (df_exp_date["FullType"] != "Intra-Account Transfert")
            ],
            index=["Date", by, label],
            values=AMOUNT_CCY,
            aggfunc="sum",
    ...
    df = pd.DataFrame(
        pd.pivot_table(
            df, index=["Year", "Month", label], values=AMOUNT_CCY, aggfunc="sum"
        ).to_records()
    )
    if len(exclude) > 0:
    ...
    if include_tail_memos:
        low_memos = list(
            [
                s
                for s in pd.pivot_table(
                    df, index=[label], values=AMOUNT_CCY, aggfunc="sum"
                )
                .sort_values(by=AMOUNT_CCY)
                .tail(show_count)
                .index
    ...
    df = pd.DataFrame(
        pd.pivot_table(
            df, index=["Year", "Month", "Memo"], values=AMOUNT_CCY, aggfunc="sum"
        ).to_records()
    ).sort_values(by=AMOUNT_CCY)
...
def plot_spend_waterfall(
    ...
    categorised_flows = pd.pivot_table(
        df_exp_filter, index="FullType", values=AMOUNT_CCY, aggfunc=sum
    )[AMOUNT_CCY]
    categorised_flows.loc["Pension"] = df_exp_filter[
        df_exp_filter.FullSubType == "UBS Pension"
    ].AmountCcy.sum()
    ...
    small_flows = (
        categorised_flows[
            (categorised_flows > -threshold)
            & (categorised_flows < threshold)
            & (categorised_flows.index != "Others")
        ].sum()
    ...
    return plot_waterfall(
        categorised_flows,
        (
            f"Income/Spending Summary {date_range[0]:%b-%y} to {date_range[1]:%b-%y}"
            if inc_title
            else None
        ),
    )
...
def plot_category_breakdown(
    ...
    fig_category_brkdn = px.bar(
        df_temp, x="Month", y=AMOUNT_CCY, color="Memo", text="Memo"
    )
