...
def project(initial, year, growth, volatility, std_dev, contribution):
    ...
    return project(initial, year, growth, volatility, std_dev, 0) + sum(
        [project(contribution, y, growth, volatility, std_dev, 0) for y in range(0, year)]
    )
    ...
    df["Forecast"] = df.apply(
        lambda row: [
            project(
                row[by],
                i,
                0.0 if ignore_growth else row["Return"],
                0.0 if ignore_growth else row["Volatility"],
                0.0,
                0.0 if ignore_contrib else row["Contribution"],
            )
            for i in r
        ],
        axis=1,
...
def project_plot_compare(w, w_low, w_up, w_low_ex, w_up_ex, w_2, w_low_2, w_up_2, w_low_ex_2, w_up_ex_2, r, ignore_fill=True):
    fig, ax = plt.subplots(figsize=(12, 6))
    formatter = FuncFormatter(thousands)
    ax.yaxis.set_major_formatter(formatter)
    ax.set(xlabel="Years", ylabel="Wealth", title="Projected Wealth")
