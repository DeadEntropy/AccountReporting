...
def load(path_in, account_name):
    ...
    df[f" Paid In ({currency}) "] = pd.to_numeric(df[f" Paid In ({currency}) "].str.replace(',',''), errors='coerce').fillna(0)
    df[f" Paid Out ({currency}) "] = pd.to_numeric(df[f" Paid Out ({currency}) "].str.replace(',',''), errors='coerce').fillna(0)

    df_out = pd.DataFrame(columns=target_columns)
    df_out.Date = pd.to_datetime(df["Completed Date "])
